...
public class QuestGiverUIScript : MonoBehaviour {
    ...
    public void SetRewardsUI(Quest quest) {
        ...
        itemText.text = "<color=#0057EF>" + quest.rewards.ItemReward.name + "</color>   " + quest.rewards.ItemRewardCount + "個";

        rewardsEXP.SetActive(true);
        rewardsEXP.transform.GetChild(0).GetComponent<Text>().text = "<color=yellow>経験値</color>   " + quest.rewards.EXPReward.ToString() + " EXP";
    }
